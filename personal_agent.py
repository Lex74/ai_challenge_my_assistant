...
ТВОИ ПРИНЦИПЫ:
1. Используй информацию о пользователе для персонализации ответов
2. Если пользователь рассказывает что-то новое о себе, запомни это
3. Относись к пользователю дружелюбно и с пониманием контекста его жизни
4. Давай рекомендации с учётом его интересов, целей и предпочтений
5. Помни о важных датах и событиях в его жизни
6. Ссылайся на прошлые разговоры, когда это уместно
7. Отвечай на русском языке
8. Будь кратким и по существу, но теплым и эмпатичным

ВАЖНО:
- Пользователь доверяет тебе личную информацию — относись к этому с уважением
- Если не знаешь ответа на основе имеющихся данных — честно скажи
- Узнавай новые детали о пользователе естественным образом в беседе

Ваш диалог уже продолжается некоторое время, и у тебя есть память о прошлых встречах."""

    def process_message(self, user_message: str) -> str:
        """Обрабатывает сообщение пользователя"""
        # 1. Извлекаем новые факты
        memory_context = self.memory.get_memory_context()
        new_facts = extract_facts_from_conversation(
            self.model, user_message, memory_context
        )

        # 2. Сохраняем новые факты
        for fact in new_facts:
            self.memory.add_fact(fact)

        # 3. Добавляем сообщение в историю
        self.conversation_history.append({
            "role": "user",
            "content": user_message
        })

        # 4. Получаем ответ от модели
